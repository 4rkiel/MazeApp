...
public class Laby extends JPanel {
	...
	public boolean resolveAux(int k) {
		...
		if (!found) {
			...
			if (((k % width) > 0) && (tmpStr.charAt(mx(k)) != 'X') && (tmpStr.charAt(mx(k)) != 'Z')) {

				res = res || resolveAux(mx(k));
			}

			if (((k % width) < width) && (tmpStr.charAt(px(k)) != 'X') && (tmpStr.charAt(px(k)) != 'Z')) {

				res = res || resolveAux(px(k));
			}

			if ((k >= width) && (tmpStr.charAt(my(k)) != 'X') && (tmpStr.charAt(my(k)) != 'Z')) {

				res = res || resolveAux(my(k));
			}

			if ((k < (width * (height - 1))) && (tmpStr.charAt(py(k)) != 'X') && (tmpStr.charAt(py(k)) != 'Z')) {
	...
	public boolean resolveBestAux(int k, int m) {
		...
		if (tmpStr.charAt(k) == 'T') {
			...
			if (((k % width) > 0) && (tmpStr.charAt(mx(k)) != 'X')
					&& ((tmpStr.charAt(mx(k)) != 'Z') || list[mx(k)] > m + 1)) {

				res = resolveBestAux(mx(k), m + 1) || res;
			}

			if (((k % width) < width) && (tmpStr.charAt(px(k)) != 'X')
					&& ((tmpStr.charAt(px(k)) != 'Z') || list[px(k)] > m + 1)) {

				res = resolveBestAux(px(k), m + 1) || res;
			}

			if ((k >= width) && (tmpStr.charAt(my(k)) != 'X')
					&& ((tmpStr.charAt(my(k)) != 'Z') || list[my(k)] > m + 1)) {

				res = resolveBestAux(my(k), m + 1) || res;
			}

			if ((k < (width * (height - 1))) && (tmpStr.charAt(py(k)) != 'X')
					&& ((tmpStr.charAt(py(k)) != 'Z') || list[py(k)] > m + 1)) {
